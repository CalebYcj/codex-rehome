...
如果恢复完成后项目中看不到旧对话，还应核对两台电脑使用的 `model_provider`（模型服务提供方标识）。部分 Codex 版本的默认对话列表会按当前提供方筛选；迁移包保留的旧标识与目标配置不同时，对话文件、索引和项目路径即使都已恢复，也可能不显示。ReHome 的恢复校验目前不检查这项列表可见性。调整前请先备份，明确要继续使用的服务，再同步处理会话元数据及本地数据库中的提供方标识；不要只修改其中一处，也不要据此认定对话已丢失。
