...
如果恢复完成后项目中看不到旧对话，还应核对两台电脑使用的 `model_provider`（模型服务提供方标识）。部分 Codex 版本的默认对话列表会按当前提供方筛选；迁移包保留的旧标识与目标配置不同时，对话文件、索引和项目路径即使都已恢复，也可能不显示。ReHome 的恢复校验目前不检查这项列表可见性。若出现 `Model provider ... not found`，请先确认目标电脑已配置对应服务；登录凭据需要在目标电脑自行设置。请保留原包和本地备份，并提供版本及隐藏个人信息的报错截图排查，不要直接批量改写会话或数据库，也不要公开配置文件、密钥或迁移包。
...
遇到导出、包检查或导入错误时，可展开“需要帮助？”。导入结果和迁移记录里也有“会话打不开？获取帮助”。

- **复制到 Codex**：先预览，再复制到这台电脑的 Codex 新对话。说明包含产品背景、本次现场位置和安全要求，不需要安装 Skill；[Agent 排查指南](docs/support/agent-recovery-guide.zh-CN.md) 也随程序内置。
- **到 GitHub 提交问题**：预览独立生成的公开摘要，再打开 GitHub，由你确认提交。不会自动上传诊断文件或迁移包。
- **重新检查数据**：只读检查本次基础文件和部分索引，不打开数据库、不自动修复。实际打开原对话并续聊仍需单独确认。

本机诊断可能包含完整路径和错误摘录；复制到 Codex 后会交给你配置的模型服务处理，请在发送前检查。公开摘要不包含本机路径、原始错误或补充描述。诊断在 ReHome 本机应用数据目录单独保存，最多 50 份；保存失败仍可复制内存摘要。旧记录缺少的证据会明确标为未验证。不要让运行中的 Codex 直接改自己的会话数据库。
