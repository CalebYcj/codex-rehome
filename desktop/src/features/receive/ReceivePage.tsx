    ...
    <div className="page receive-page">
      ...
      {plan && (
        <section className="workflow-section" aria-labelledby="restore-plan-title">
          ...
          <label className="confirmation-row"><input type="checkbox" checked={codexClosed} onChange={(event) => setCodexClosed(event.target.checked)} aria-label="确认 Codex 已关闭" /><span><strong>Codex 已关闭</strong><small>恢复会更新本机对话索引和线程数据库。</small></span></label>
